var radvizRegular = function () {
			...
			.on('mouseover', function(d) {
				...
				d3.selectAll('circle.dot_')
					.filter(function(dn,i) {
						return config.labelSet.indexOf(d) == config.colors[i];})
					.transition()
					.duration(300)
					.attr('fill-opacity',1);
				}
    ...
    var addNormalizedValues = function(data) {
		...
		function resizeElements(newsize){
			d3.selectAll('.dot_').attr("r", function(d){return +d['elemSize']*newsize+2;});			
		}
